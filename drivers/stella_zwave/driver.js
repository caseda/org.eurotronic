	...
	if (node) {
		...
		node.instance.CommandClass['COMMAND_CLASS_THERMOSTAT_MODE'].on('report', (command, report) => {
			...
			report.Level.hasOwnProperty("Mode")) {
				...
				Homey.manager('flow').triggerDevice('stella_euro_mode_changed', data, null, node.device_data);
				Homey.manager('flow').triggerDevice('stella_euro_mode_changed_to', null, data, node.device_data);
			}
		});
	}
});
