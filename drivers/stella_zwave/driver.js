		...
		eurotronic_manual_value: {
			...
			command_report_parser: (report, node) => {
				...
				if (typeof report.Value === 'number') {
					if (node) Homey.manager('flow').triggerDevice('stella_euro_manual_position', { value: report.Value / 100 }, null, node.device_data);
				...
				if (typeof report['Value (Raw)'] !== 'undefined') {
					...
					if (node) Homey.manager('flow').triggerDevice('stella_euro_manual_position', { value: report['Value (Raw)'][0] / 100 }, null, node.device_data);
